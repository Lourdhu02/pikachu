...
def test_forward_pass():
    model = YOLO("yolo26m-obb.pt")
    dummy = np.random.randn(640, 640, 3).astype(np.float32)
    results = model.predict(dummy, imgsz=640, verbose=False)
...
def test_output_shape():
    ...
    for r in results:
        if r.obb is not None:
            ...
            assert boxes.shape[-1] == 2 and boxes.shape[-2] == 4, f"OBB format: expected (N, 4, 2), got {boxes.shape}"
            confs = r.obb.conf
            assert confs.ndim == 1, "Confidences should be 1D"
